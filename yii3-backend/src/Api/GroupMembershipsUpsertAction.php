...
final readonly class GroupMembershipsUpsertAction
{
    public function __invoke(
        ...
        try {
            ...
            if ($id !== null) {
                $row = $memberships->updateGroup((string) $auth['team_id'], $id, $permission);
            } else {
                $groupId = RequestData::string($body, 'groupId');
                if ($groupId === null) {
                    throw new RuntimeException('groupId is required.');
                }
                $row = $memberships->createGroup(
                    (string) $auth['team_id'],
                    $groupId,
        ...
        return $responseFactory->createResponse([
            'data' => MembershipPresenter::group($row),
