...
router.post(
  ...
  auth(),
  validate(T.UsersUpdatePasswordSchema),
  transaction(),
  async (ctx: APIContext<T.UsersUpdatePasswordReq>) => {
    const { transaction } = ctx.state;
    const actor = ctx.state.auth.user;
    const { id, currentPassword, password } = ctx.input.body;
    ...
    // A user changing their own existing password must confirm the old one.
    // Administrators may reset another user's password without knowing it.
    if (
      actor.id === user.id &&
      user.passwordHash &&
      (!currentPassword ||
      ...
      throw InvalidAuthenticationError("Current password is incorrect");
    }
    ...
    // Invalidate all active sessions after an administrator resets another user.
    if (actor.id !== user.id) {
